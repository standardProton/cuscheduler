import { SEMESTER_END, SEMESTER_START, YEAR_DB_INFO } from "./json/consts";
...
export async function getPreScheduleClass(name, cors_anywhere){
    const YEAR_DB_INFO = 2241; //spring classes released oct 12
    const res1 = await fetch(cors_anywhere + "https://classes.colorado.edu/api/?page=fose&route=search&alias=" + name.toLowerCase(), {
        method: "POST",
        body: '{"other":{"srcdb":"' + YEAR_DB_INFO + '"},"criteria":[{"field":"alias","value":"' + name.toLowerCase() + '"}]}'
    });
