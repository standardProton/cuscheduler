...
export function renderScheduleSVG(width, height, schedule, color_key, setColorKey, scheduleClick, scheduleHover, options){
    ...
    const getY = (i) => {return ((i*h/daylen) + marginy)}
            ...
            <g>
                {Array.from(new Array(5), (x, i) => i).map(xc => (<g key={"click-event-set-" + xc}>
                    {Array.from(new Array(Math.trunc(daylen*12)), (x, i) => i).map(yc => (
                        <rect x={getX(xc) + "%"} y={getY(yc/12)+ "%"} width={(w/5) + "%"} height={(h/daylen/12) + "%"} onMouseDown={() => scheduleClick(xc, yc)} onMouseOver={() => scheduleHover(xc, yc)} key={"click-event-" + xc + "-" + yc} style={{fill: "rgba(0, 0, 0, 0)"}}></rect>
                    ))}
                </g>))}
            </g>
        </svg>
...
export function timeString(day, start, end){ //model time
    ...
    return day_str[day] + " " + ((Math.trunc(start/12)+8) % 12) + ":" + twoDigit((start % 12) * 5) + (start >= 60 ? "p" : "a") + "-" + ((Math.trunc(end/12)+8) % 12) + ":" + twoDigit((end % 12) * 5) + (end >= 60 ? "p" : "a");
}
