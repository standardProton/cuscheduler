...
export default async function handler(req, res){
    var model = {
        "optimize": "cost", //cost can involve unpreferred hours, waitlist, bad professors, etc.
        "opType": "min",
        "constraints":{
            "d0-t0": {"min": 0, "max": 1},
            "d0-t1": {"min": 0, "max": 1},
            ...
            "enrolled-count": {"min": 3, "max": 3}
        },
        "variables":{
            "c0-d0-t0": {"enrolled-count": 1, "d0-t0": 1, "c0-enrolled": 1, "cost": 0 + Math.random()},
            "c0-d0-t1": {"enrolled-count": 1, "d0-t1": 1, "c0-enrolled": 1, "cost": 0 + Math.random()},
    ...
    var f = {result: solver.Solve(model)}
    f.duration = (new Date()).getTime() - start;
